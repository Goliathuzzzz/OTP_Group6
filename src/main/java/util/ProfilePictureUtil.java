...
public class ProfilePictureUtil {
    ...
    private static Image getProfilePicture(int participantId) {
        ...
        Image image = new Image(
                Objects.requireNonNull(ProfilePictureUtil.class.getResourceAsStream(imagePath)));
        if (image.isError()) {
            System.err.println(
                    "ERROR: Error loading image: " + imagePath + ". Using default image.");
            image = new Image(Objects.requireNonNull(ProfilePictureUtil.class.getResourceAsStream(
                    "/images/" + PROFILE_PICTURES[0])));
        }
        return image;
    }
